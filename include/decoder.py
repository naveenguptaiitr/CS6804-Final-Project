    ...
    for i in range(len(num_channels_up)-1):
        ...
        if upsample_first:
            ...
            if upsample_mode!='none' and i!=0:
                ...
                #model.add(nn.Conv2d(num_channels_up[i], num_channels_up[i], kernel_size=3, padding=1))
                
                # model.add(nn.PixelShuffle(upscale_factor=2))
                # model.add(conv(num_channels_up[i]//4, num_channels_up[i+1],5, pad=pad))
                # model.add(nn.Conv2d(num_channels_up[i]//4, num_channels_up[i+1], kernel_size=3, padding=1))
